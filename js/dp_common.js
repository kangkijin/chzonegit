...
jQuery.fn.serializeFormFile = function( ) {
	...
	// formData.append("json",JSON.stringify(jsonData)); // 이런식으로 JSON 따로 선언해서 보내주면, ArgumentResolver에서 jackson 파싱해서 사용하는 방식으로 수정 예정
 ...
 * @returns
 */
function fnSetDeptCdCombo(deptDiv, viewId, selValue, allValue, useAsync, basePathClient, upDeptCd, cdCol, year) {
	if(typeof deptDiv === "object") {
		({deptDiv, viewId, selValue, allValue, useAsync, basePathClient, upDeptCd, cdCol, year} = deptDiv);
	}
	...
	jQuery.ajax({
		...
		success: function(r) {
			if (r.beanlist.length > 0) {
				...
				if( chkdValue == "" || chkdValue == null ) targetView.find("input").eq(0).prop("checked", true);
			}
		},
		error: function(r) {
			console.log("공통코드 조회중 오류가 발생하였습니다.("+comnCdPcodeId+")");
